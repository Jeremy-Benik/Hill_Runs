...
import wrf
import os
fire_atms_1 = nc.Dataset(
    '/Users/jeremybenik/Research_files/hill_run/cheyenne_runs/run_1/wrfout_d01_0001-01-01_00:00:00')
fire_atms_0 = nc.Dataset(
    '/Users/jeremybenik/Research_files/hill_run/cheyenne_runs/fire_atms_0/wrfout_d01_0001-01-01_00:00:00')
...
# The second height was picked as that is 5.19m in the ff2 file
...
for i in range(-1, 180):
    im = plt.contourf(
        lon_fire, lat_fire, fire_p[i, 0, :, :], levels, cmap='seismic', shading='auto')
    plt.contour(lon_fire, lat_fire,
                fire[i], colors="black", linewidths=0.8, antialiased=True)
    cb = plt.colorbar(im)
    cb.set_label('Pressure (hPa)', fontsize=12, fontweight='bold')
    plt.quiver(lon_fire[0, ::3], lat_fire[::3, 0], u_fire[i, 2, :, :]
               [::3, ::3], v_fire[i, 2, :, :][::3, ::3], scale=None)
    plt.suptitle('Surface Pressure with Wind Vectors',
    ...
    if i == -1:
        ...
        output_path = os.path.join(
            '/Volumes/Data/hill_run_data/hill_run/images/Plot_1', str(i) + '.png')
...
levels = np.linspace(0, abs(fire_wind[:]).max(), 101)
fig, ax = plt.subplots(figsize=(10, 8), dpi=200)
for i in range(-1, 180):
    im = plt.contourf(lon_fire, lat_fire, fire_wind[i, 2, :, :], levels, 
                      cmap='Reds', shading='auto')
    # Change colourmap from something bright to dark
    plt.contour(lon_fire, lat_fire,
                fire[i], colors="black", linewidths=0.8, antialiased=True)
    cb = plt.colorbar(im)
    ...
    if i == -1:
        ...
        output_path = os.path.join(
            '/Volumes/Data/hill_run_data/hill_run/images/Plot_2', str(i) + '.png')
        # plt.savefig(output_path,bbox_inches='tight',dpi=150)
        plt.show()
        plt.close(fig)
# %% Plot 3
print('Creating the third plot')
levels = np.linspace(-abs(w_fire[:]).max(), abs(w_fire[:]).max(), 101)
fig, ax = plt.subplots(figsize=(10, 8), dpi=200)
for i in range(-1, 180):
    im = plt.contourf(lon_fire, lat_fire, w_fire[i, 2, :, :], levels, cmap='seismic', shading='auto')
    plt.contour(lon_fire, lat_fire,
                fire[i], colors="black", linewidths=0.8, antialiased=True)
    cb = plt.colorbar(im)
    cb.set_label('W Wind Speed (m/s)', fontsize=12)
    plt.suptitle('Vertical Velocities with Fire Wind Vectors',
                 fontsize=12, fontweight='bold')
    plt.grid()
    plt.title('Time=%d' % i)
    plt.xlabel('Lon', fontsize=12, fontweight='bold')
    ...
    if i == -1:
        ...
        output_path = os.path.join(
            '/Volumes/Data/hill_run_data/hill_run/images/Plot_3', str(i) + '.png')
        #plt.savefig(output_path, bbox_inches='tight', dpi=150)
        plt.show()
        plt.close(fig)
